...
class TeamsPresenter(
    ...
    fun getSearch(queryId: String?){
        teamsView.showLoading()
        GlobalScope.launch(context.main){
            val query = gson.fromJson(apiRepository.doRequest(ApiServices.getSearchTeams(queryId)).await(), TeamResponse::class.java)
            teamsView.hideLoading()
            teamsView.showListTeam(query.bdageTeam)
        }
    }
}